...
function CustomFilter({title, options} : Props) {
  ...
  return (
    <div className="w-fit">
      <Listbox value={selected} onChange={(e) => { setSeleted(e); handleUpdateParams(e)}}>
        <div className="relative w-fit z-10">
          ...
          >
            <Listbox.Options className="custom-filter__options">
              {options.map(opt => (
                <Listbox.Option key={opt.title} value={opt} className={({active}) =>`relative cursor-default select-none py-2 px-4 ${active ? "bg-primary-blue text-white" : "text-gray-900"}`}>
                  {({selected}) => (
